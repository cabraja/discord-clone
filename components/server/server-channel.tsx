...
function ServerChannel({channel,server,role}:ServerChannelProps) {
  ...
  return (
    ...
    >
        ...
        <p className={cn('line-clamp-1 font-semibold text-sm text-zinc-500 group-hover:text-zinc-600 dark:text-zinc-400 dark:group-hover:text-zinc-300',
        params?.channelId === channel.id && "text-primary dark:text-zinc-200 dark:group-hover:text-zinc-400"
        )}>
            {channel.name}
        </p>
